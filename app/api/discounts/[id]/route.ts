...
const updateDiscountSchema = z.object({
  ...
  value: z
    ...
    .pipe(z.number().min(0, "La valeur doit être positive").optional()),
  minAmount: z
    .string()
    .transform((val) => (val === "" ? undefined : parseFloat(val)))
    .pipe(z.number().min(0, "Le montant minimum doit être positif").optional()),
  maxUses: z
    .string()
...
export async function PATCH(
  ...
  try {
    ...
    if (validatedData.minAmount !== undefined)
      updateData.minAmount = validatedData.minAmount || null;
    if (validatedData.maxUses !== undefined)
      updateData.maxUses = validatedData.maxUses || null;
    if (validatedData.isActive !== undefined)
      updateData.isActive = validatedData.isActive;
    if (validatedData.startsAt !== undefined)
      updateData.startsAt = validatedData.startsAt || null;
    if (validatedData.expiresAt !== undefined)
      updateData.expiresAt = validatedData.expiresAt || null;
